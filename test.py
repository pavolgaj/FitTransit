...
limits={'t0':[t0-1e-2,t0+1e-2],'P':[14.5,15.5],'Rp':[5e-3,5e-2],'a':[15,20],'i':[85,90],'e':[0,0.1],'w':[0,360],'c1':[0.1,0.2],'c2':[0.4,0.5],'p0':[-1.0251,-1.0249],'p1':[8e-2,0.1],'p2':[-5e-3,0]}
steps={'t0':1e-4,'P':1e-2,'Rp':1e-5,'a':0.1,'i':0.1,'e':1e-2,'w':0.1,'c1':1e-3,'c2':1e-3,'p0':1e-6,'p1':1e-3,'p2':1e-4}
...
mpl.plot((t-t0)*24,flux0)
mpl.plot((t-t0)*24,flux1)

#mpl.figure()
#mpl.plot((t-t0)*24,flux0)
#mpl.plot((t-t0)*24,flux0/pp+pp-1)

#mpl.figure()
#mpl.plot((t-t0)*24,flux0-(flux0/pp+pp-1))
...
#tf.Normalise([0,3e6])
tf.Normalise([t0-0.15,t0+0.15],reverse=True,plot=True)
tf.params['p0']=1
tf.params['p1']=0
tf.params['p2']=0
tf.Plot()
